...
if [ "$PRUNE_GETH" != "" ]; then
	...
	if [ "$NETWORK" != "$DEFAULT_NETWORK" ]; then
		...
		exec geth --syncmode snap --http --http.addr "0.0.0.0" --http.vhosts=* --http.api "eth,net" --ipcdisable --datadir.ancient=/root/ancient-data --authrpc.addr "0.0.0.0" --authrpc.port "8551" --authrpc.vhosts "*" --authrpc.jwtsecret "/root/jwttoken"
	fi
fi
